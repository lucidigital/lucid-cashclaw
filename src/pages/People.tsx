...
const emptyForm = (): Omit<Person, 'id' | 'createdAt'> => ({
  name: '', type: 'staff', role: '', phone: '', taxCode: '', bankInfo: '', baseSalary: 0, note: '',
});
...
export default function People() {
  ...
  // ─── Tab state ────────────────────────────
  const [activeTab, setActiveTab] = useState<'people' | 'salary'>('people');
  ...
  function openEdit(p: Person) {
    setEditing(p);
    setForm({ name: p.name, type: p.type, role: p.role || '', phone: p.phone || '', taxCode: p.taxCode || '', bankInfo: p.bankInfo || '', baseSalary: p.baseSalary || 0, note: p.note || '' });
    setModalOpen(true);
  }
  ...
  return (
    <div className="people-page animate-fade-in">
      ...
      <div className="people-tabs">
        <button
          className={`ptab-btn ${activeTab === 'people' ? 'active' : ''}`}
          onClick={() => setActiveTab('people')}
        >
          👥 Danh sách nhân sự
        </button>
        <button
          className={`ptab-btn ${activeTab === 'salary' ? 'active' : ''}`}
                ...
                {(form.type === 'leader' || form.type === 'staff') && (
                  <div className="form-group">
                    ...
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: 4, display: 'block' }}>
                      Sẽ tự điền vào bảng lương hàng tháng
                    </span>
                  </div>
                )}
